...
Variables runtime auth/users/quote/schedule:
...
- `SCHEDULE_SERVICE_ADDRESS` a ajouter dans le gateway
- service `devis-schedule` a ajouter
...
## Ajouts cibles pour le service schedule

### Cible compose local backend

Ajouts attendus:

- service `devis-schedule`
- `SCHEDULE_SERVICE_ADDRESS=devis-schedule:50056` dans le gateway
- `QUOTE_SERVICE_ADDRESS=devis-quote:50053` dans le service schedule
- variables DB du service schedule:
  - `DB_USER=devis-schedule`
  - `DB_NAME=schedule`

### Cible compose production
...
- variables DB du service schedule:
  - `DB_USER=devis-schedule`
  - `DB_NAME=${SCHEDULE_DB_NAME:-schedule}`
