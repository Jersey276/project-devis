...
| INV-BE-041 | Unit           | Combinaisons (opt-in, cumul, N-1, type client, pays)   | Decider l'application OSS           | Vrai si B2C + UE hors FR + (opt-in OU cumul N >= seuil OU N-1 >= seuil) | P0       | `oss_threshold_test.go:TestOSSApplies`                       |
...
| INV-BE-074 | Integration DB | Ventes N-1 >= seuil, zero vente N ; puis avoir N-1     | Regle annee N-1 (C3)                | N-1 over=vrai des le 1er euro (cumul N=0, legs disjoints) ; avoir N-1 deduit fait repasser sous le seuil | P0 | `oss_threshold_integration_test.go:TestOSSPriorYear_OverThreshold` |
...
| INV-BE-083 | Unit           | Adaptateur Iopole, serveur HTTP simule, 201 `{id}` | Deposer (`Submit`)          | `multipart/form-data` champ `file`, header `Authorization: Bearer` + `customer-id`, `SubmissionID=id`, statut `SUBMITTED` | P0 | `pdp/iopole/iopole_test.go:TestSubmit_DepositsFacturXAndReturnsID` |
| INV-BE-084 | Unit           | Adaptateur Iopole, reponse 413                 | Deposer                         | Erreur remontee (non-201)                                                | P1       | `pdp/iopole/iopole_test.go:TestSubmit_Non201IsError` |
| INV-BE-085 | Unit           | DocumentSource en erreur                       | Deposer                         | Erreur avant tout appel reseau ; la PA n'est jamais appelee              | P0       | `pdp/iopole/iopole_test.go:TestSubmit_DocumentErrorAbortsBeforeNetwork` |
| INV-BE-086 | Unit           | `status-history` non ordonne                   | Interroger le statut (`FetchStatus`) | Prend l'item le plus recent par date et le mappe (`APPROVED`)        | P0       | `pdp/iopole/iopole_test.go:TestFetchStatus_PicksMostRecentAndMaps` |
| INV-BE-087 | Unit           | `status-history` vide                          | Interroger le statut            | `UNKNOWN` (aucune transition)                                            | P1       | `pdp/iopole/iopole_test.go:TestFetchStatus_EmptyHistoryIsUnknown` |
...
| INV-PDF-006 | Integration | Parties avec SIRET                      | Generer Factur-X      | Id legal schema `0009` (SIRET) ; `0002` (SIREN) omis      | P1       | Couvert (service `export`) |
