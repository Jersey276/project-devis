...
// SIREN/VAT added by migration 000004.
func seedPartySnapshot(t *testing.T, db *sql.DB, invoiceID, clientSiren, clientVat string) {
	t.Helper()
	_, err := db.Exec(
		`INSERT INTO invoice_party_snapshots (
			...
			client_first_name, client_last_name, client_company, client_siren, client_vat
		          ...
		          'Jean', 'Dupont', 'Dupont SAS', $2, $3)`,
		invoiceID, clientSiren, clientVat)
	if err != nil {
...
func TestPartySnapshot_ExposesClientTaxIds(t *testing.T) {
	...
	seedPartySnapshot(t, db, "inv-party", "987654321", "FR99887766554")
