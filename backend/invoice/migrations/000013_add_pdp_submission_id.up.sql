-- Platform handle returned by the PA (Plateforme Agréée) on deposit (B6). NULL
-- until deposited, or when the no-op adapter is in use. Nullable + additive: no
-- backfill, no constraint. It is the only handle a future status poll can use.
ALTER TABLE invoices ADD COLUMN pdp_submission_id TEXT;

-- Fix a latent inalterability bug and extend it for B6. The 000003 trigger only
-- whitelisted the ISSUED→PAID move on a sealed invoice, so advancing
-- lifecycle_status (added in 000011, B3) was rejected as "sealed". Operational
-- e-invoicing metadata (lifecycle_status, pdp_submission_id) is orthogonal to the
-- immutable legal document: it may change on a sealed row provided every legal /
-- financial column stays frozen. Both allowed paths share that frozen-column
-- guard, so the document itself remains inalterable (chained seal still holds).
...
BEGIN
    ...
    -- operational columns (lifecycle_status, pdp_submission_id) are intentionally
    -- excluded so the two branches below can vary them.
    legal_frozen :=
    ...
    -- lifecycle_status / pdp_submission_id / updated_at may differ.
    IF legal_frozen
