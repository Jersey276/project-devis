...
func (s *Server) ListSchedules(ctx context.Context, req *scheduleGrpc.ListSchedulesRequest) (*scheduleGrpc.ListSchedulesResponse, error) {
	if req == nil || strings.TrimSpace(req.UserId) == "" {
		return &scheduleGrpc.ListSchedulesResponse{Success: false, Code: CodeInvalidInput}, nil
	}
	...
	if err != nil {
		return &scheduleGrpc.ListSchedulesResponse{Success: false, Code: CodeInternalError}, err
	}
	defer rows.Close()
	...
	return &scheduleGrpc.ListSchedulesResponse{
		Success:   true,
		Code:      CodeSuccess,
		Schedules: schedules,
	}, nil
