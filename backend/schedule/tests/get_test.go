...
func TestGetSchedule_Success(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT quote_id, status, name, start_month, duration_months, user_id, COALESCE\(client_id, ''\) FROM schedules WHERE schedule_id=\$1 AND user_id=\$2`).
		WithArgs("schedule-1", "user-1").
		WillReturnRows(sqlmock.NewRows([]string{"quote_id", "status", "name", "start_month", "duration_months", "user_id", "client_id"}).
			AddRow("quote-1", actions.StatusDraft, "Echeancier principal", time.Date(2026, time.June, 1, 0, 0, 0, 0, time.UTC), int32(2), "user-1", "client-1"))
...
func TestGetSchedule_ByClientId_Success(t *testing.T) {
	srv, mock := setupServer(t)
	restore := actions.SetQuoteLineExpectedCentsFetcherForTests(func(context.Context, string, string) (map[string]int64, error) {
	...
	mock.ExpectQuery(`SELECT quote_id, status, name, start_month, duration_months, user_id, COALESCE\(client_id, ''\) FROM schedules WHERE schedule_id=\$1 AND client_id=\$2`).
		WithArgs("schedule-1", "client-1").
		WillReturnRows(sqlmock.NewRows([]string{"quote_id", "status", "name", "start_month", "duration_months", "user_id", "client_id"}).
			AddRow("quote-1", actions.StatusValid, "Echeancier principal", time.Date(2026, time.June, 1, 0, 0, 0, 0, time.UTC), int32(1), "user-1", "client-1"))
	...
	mock.ExpectQuery(`SELECT month_index, COALESCE\(SUM\(amount_cents\), 0\) FROM schedule_cells WHERE schedule_id=\$1 GROUP BY month_index ORDER BY month_index`).
		WithArgs("schedule-1").
		WillReturnRows(sqlmock.NewRows([]string{"month_index", "amount_cents"}).
			AddRow(int32(1), int64(1000)))

	// No user_id — this is how the gateway calls GetSchedule in customer mode.
	resp, err := srv.GetSchedule(context.Background(), &scheduleGrpc.GetScheduleRequest{
		ScheduleId: "schedule-1",
		ClientId:   "client-1",
	})
	...
	if resp.Schedule.ClientId != "client-1" {
		t.Fatalf("expected client_id client-1, got %q", resp.Schedule.ClientId)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatalf("unmet expectations: %v", err)
	}
