...
func TestGetUser_Success(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT user_id, email`).
		WithArgs("user-123").
		WillReturnRows(sqlmock.NewRows([]string{"user_id", "email", "phone", "company", "siren", "vat", "logo_url", "suspended", "oss_enabled"}).
...
func TestUpdateUser_Success(t *testing.T) {
	...
	mock.ExpectExec(`UPDATE users SET phone`).
		WithArgs("0600000000", "ACME", "123456789", "FR12345", "https://cdn.example.com/logo.png", true, "user-123").
		WillReturnResult(sqlmock.NewResult(1, 1))
