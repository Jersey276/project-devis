...
func Get(ctx context.Context, db *sql.DB, req *usersGrpc.GetUserRequest) (*usersGrpc.GetUserResponse, error) {
	...
	err := db.QueryRowContext(ctx,
		"SELECT user_id, email, COALESCE(phone,''), COALESCE(company,''), COALESCE(siren,''), COALESCE(vat,''), suspended, oss_enabled, COALESCE(iban,''), COALESCE(bic,''), COALESCE(siret,''), COALESCE(first_name,''), COALESCE(last_name,'') FROM users WHERE user_id = $1",
		req.UserId,
	).Scan(&u.UserId, &u.Email, &u.Phone, &u.Company, &u.Siren, &u.Vat, &u.Suspended, &u.OssEnabled, &u.Iban, &u.Bic, &u.Siret, &u.FirstName, &u.LastName)
