...
func Get(ctx context.Context, db *sql.DB, req *quoteGrpc.GetQuoteLineRequest) (*quoteGrpc.GetQuoteLineResponse, error) {
	...
	err := db.QueryRowContext(ctx,
		`SELECT l.line_id, l.quote_id, l.type, l.name, l.quantity::text, COALESCE(l.unit, ''), l.unit_price, l.data::text, l.position
		 FROM quote_lines l
		 JOIN quotes q ON q.quote_id = l.quote_id
		 WHERE l.line_id=$1 AND q.user_id=$2`,
		req.LineId, req.UserId,
	).Scan(&l.LineId, &l.QuoteId, &l.Type, &l.Name, &l.Quantity, &l.Unit, &l.UnitPrice, &l.Data, &l.Position)
