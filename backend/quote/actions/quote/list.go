...
func List(ctx context.Context, db *sql.DB, req *quoteGrpc.ListQuotesRequest) (*quoteGrpc.ListQuotesResponse, error) {
	...
	query := `SELECT quote_id, user_id, name, archived_at, state, client_id, address_id FROM quotes WHERE user_id=$1`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&quoteID, &userID, &name, &archivedAt, &state, &clientID, &addressID); err != nil {
			return &quoteGrpc.ListQuotesResponse{Success: false, Code: codes.InternalError}, err
		}
