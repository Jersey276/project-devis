...
func (s *Server) ListPlans(ctx context.Context, req *subscriptionGrpc.ListPlansRequest) (*subscriptionGrpc.ListPlansResponse, error) {
	query := `SELECT plan_id, name, tier, price_cents, billing_cycle, features::text, active, COALESCE(stripe_price_id, '')
	          FROM plans`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.PlanId, &p.Name, &p.Tier, &p.PriceCents, &p.BillingCycle, &p.Features, &p.Active, &p.StripePriceId); err != nil {
			return &subscriptionGrpc.ListPlansResponse{Success: false, Code: CodeInternalError}, nil
		}
